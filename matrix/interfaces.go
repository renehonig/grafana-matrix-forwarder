...
type Writer interface {
	...
	Send(ctx context.Context, roomID string, body FormattedMessage) (string, error)
	// Reply to the provided event ID with the provided message, returns the event ID if successful
	Reply(ctx context.Context, roomID string, eventID string, body FormattedMessage) (string, error)
	// React to a given event ID, returns the new event ID if successful
	React(ctx context.Context, roomID string, eventID string, reaction string) (string, error)
}
